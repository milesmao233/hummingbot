...
def signal(*args):
    ...
    df[f'{column_name}_pct_change'] = df[column_name].pct_change(std_interval)
    df[f'{column_name}_pct_change_std'] = calculate_rolling_std(df[f'{column_name}_pct_change'], period, std_interval)

    # df[f'{column_name}_pct_change'] = df[column_name].pct_change(std_interval)
    # df[f'{column_name}_pct_change_std'] = df[f'{column_name}_pct_change'].rolling(period).std(ddof=0)
    df[f'{column_name}_pct_change_last'] = df[column_name].pct_change(std_interval).shift()
    df[f'{column_name}_pct_change_std_last'] = (
        calculate_rolling_std(df[f'{column_name}_pct_change_last'], period, std_interval))
    df[factor_name] = (
            df[f'{column_name}_pct_change_std'] / df[f'{column_name}_pct_change_std_last'] - 1)

    del df[f'{column_name}_pct_change'], df[f'{column_name}_pct_change_std']
    del df[f'{column_name}_pct_change_last'], df[f'{column_name}_pct_change_std_last']
